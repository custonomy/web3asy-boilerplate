...
import { IMap, INFT, IErrorResponse } from "../utils/types";
...
const CreditCardModal: React.FC<CreditCardModalProps> = (props) => {
  ...
  const [errors, setErrors] = useState<IMap>({});
  ...
  const handleSubmit = async () => {
    handleNextStep();
    setIsLoading(true);
    // send mint request (NOTE: THIS APPROACH OF MINTING IS FOR DEMO ONLY, NOT A RECOMMENDED WAY)
    try {
      const result = await mint({ address: credentials.address, asset: props.nft.asset });
      setTxnHash(result.hash);
    } catch (err: unknown) {
      setErrors({ ...errors, request: (err as AxiosError<IErrorResponse>)?.response?.data?.error ?? "Failed to mint the NFT 😕" });
    }
    setIsLoading(false);
  };
  ...
  return (
    <Dialog
      ...
      PaperProps={{ sx: isTablet ? {} : { height: "100%", maxHeight: "none", margin: 0, borderRadius: 0, maxWidth: 850 } }}
      sx={{ "& .MuiDialog-container": { justifyContent: isTablet ? "center" : "flex-end" } }}
    >
      <Grid container height="100%" width="100%">
        <Grid item sx={{ background: "#f2f2f2" }} p={4} textAlign="center" width={isTablet ? "100%" : "auto"}>
          ...
          <Box component="div" sx={{ background: "white", borderRadius: 2, px: 2, py: 1, my: 2, fontSize: 18, width: "fit-content", margin: "auto" }}>
          ...
          <Box component="div" sx={{ background: "white", borderRadius: "10px !important", px: 2, py: 1, my: 3, fontSize: 18 }}>
        ...
        <Grid
          ...
          sx={{ boxShadow: isTablet ? "-10px -10px 10px 1px #d9d9d9" : "-10px 0px 10px 1px #d9d9d9" }}
        >
          <Grid container flexDirection="column" alignItems="center" justifyContent="space-between" height="100%">
            <Grid item height={step === 1 ? "90%" : "auto"} width="100%">
              <Grid container alignItems="center" mt={2} px={3}>
                {step === 0 && (
                  <Box component="div" display="flex" alignItems="center" justifyContent="center" sx={{ background: "#f2f2f2", borderRadius: 100, p: 1, mr: 1 }}>
                ...
                {step === 1 && (
                  <Box display="flex" alignItems="center" justifyContent="center" component="div" sx={{ background: "#f2f2f2", borderRadius: 100, p: 1, mr: 1 }}>
              ...
              {step === 0 && (
                <Box component="div" px={4}>
                  ...
                  <Box component="div" sx={{ borderRadius: 2, border: "2px solid black", p: 2 }}>
                    ...
                    <Typography sx={{ color: "rgba(0,0,0,.6)" }}> Mint this NFT to your own walletContext.</Typography>
                    <TextField
                      fullWidth
                  ...
                  {isLoading ? (
                    ...
                    <Box component="div" px={4}>
                      <Typography variant="h6" component="div" display="flex" flexDirection="column" alignItems="center" justifyContent="center" pb={4}>
                        {txnHash ? "Your NFT has been minted! 🎉" : errors.request ?? "Something went wrong."}
                      </Typography>
                    </Box>
                  )}
                </Box>
            ...
            <Grid item width="100%" px={4}>
              {step === 0 && (
                <Button
                  ...
                  color="secondary"
                  size="large"
                  sx={{ borderRadius: 3, mb: 5, py: 1, fontSize: 18, mt: isTablet ? 5 : 0 }}
                  ...
                  {txnHash ? (
                    <>
                      <Button
                        ...
                        color="secondary"
                        size="large"
                        sx={{ borderRadius: 3, mb: 2, py: 1, fontSize: 18, mt: isTablet ? 4 : 0 }}
                      ...
                      <Button
                        ...
                        color="secondary"
                        size="large"
                        sx={{ borderRadius: 3, mb: 5, py: 1, fontSize: 18, mt: isTablet ? 2 : 0 }}
