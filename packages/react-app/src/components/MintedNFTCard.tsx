...
const StyledImage = styled("img")({
  ...
  transition: "0.3s all",
  "&:hover": {
    boxShadow: "rgba(0, 0, 0, 0.25) 0px 14px 28px, rgba(0, 83, 138, 0.4) 0px 10px 10px",
...
const MintedNFTCard: React.FC<MintedNFTCardProps> = (props) => {
  ...
  const handleViewNFT = () => {
    window.open(`https://${NETWORK === "mumbai" ? "mumbai." : ""}polygonscan.com/token/${props.nft.contract.address}?a=${parseInt(props.nft.id.tokenId, 16)}`);
  };
  ...
  return (
    <Grid container flexDirection="column" justifyContent="center">
      <StyledImage src={parseIPFSUrl(props.nft.metadata.image)} alt={props.nft.title} height={isMobile ? 200 : 300} onClick={handleViewNFT} />
